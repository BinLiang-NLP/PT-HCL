...
class Instructor:
    ...
    def _train(self, sup_criterion, pretext_criterion, stance_criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            val_acc, val_f1, val_f1_pro,val_f1_con, val_f1_avg = self._evaluate_acc_f1(val_data_loader)
            logger.info('> val_acc: {:.4f}, val_f1: {:.4f}, val_f1_pro: {:.4f}, val_f1_con: {:.4f}, val_f1_avg: {:.4f}'.format(val_acc, val_f1, val_f1_pro,val_f1_con, val_f1_avg))
            if val_acc > max_val_acc:
                max_val_acc = val_acc
            if val_f1_avg > max_val_f1_avg:  # sem16
                max_val_f1_avg = val_f1_avg
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        f1_pro = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[2], average='macro')
        f1_con = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0], average='macro')
        f1_avg = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 2], average='macro')

        return acc, f1, f1_pro, f1_con, f1_avg
    ...
    def run(self):
        ...
        test_acc, test_f1, test_f1_pro, test_f1_con,test_f1_avg = self._evaluate_acc_f1(test_data_loader)
        logger.info('>> test_acc: {:.4f}, test_f1: {:.4f}, test_f1_pro: {:.4f}, test_f1_con: {:.4f},test_f1_avg: {:.4f}'.format(test_acc, test_f1, test_f1_pro, test_f1_con, test_f1_avg))
        f_out = open('log/' + self.opt.dataset+'_' + str(self.opt.seed)+ '.txt', 'w', encoding='utf-8')
        f_out.write('test_acc: {:.4f}, test_f1: {:.4f}, test_f1_pro: {:.4f},test_f1_con: {:.4f}, test_f1_avg: {:.4f}'.format(test_acc, test_f1, test_f1_pro,test_f1_con, test_f1_avg)+'\n')
        return test_f1_pro, test_f1_con, test_f1_avg 

def main():
    # Hyper Parameters
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_name', default='bert_pt_hcl', type=str)
    parser.add_argument('--dataset', default='hc', type=str)
    ...
    ins = Instructor(opt)
    now_f1_pro, now_f1_con, now_f1_avg = ins.run()
