...
class Instructor:
    ...
    def _train(self, criterion, con_criterion, optimizer, train_data_loader, val_data_loader):
        ...
        global_step = 0
        path = None
        f_al_un = open('log/align_uniform_record_vast.txt', 'w', encoding='utf-8')
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            n_correct, n_total, loss_total, sub_loss1, sub_loss2, sub_loss3,al_loss_total,un_loss_total = 0, 0, 0, 0, 0, 0,0,0
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                polarity=batch['polarity'].detach().numpy().tolist()
                cross=batch['cross_label'].detach().numpy().tolist()
                hidden1_list=hidden1.cpu().detach().numpy().tolist()
                hidden2_list=hidden2.cpu().detach().numpy().tolist()
                for i in range(len(polarity)):
                    temp_string=targets_dict[polarity[i]]+'-'+cross_dict[cross[i]]+'\t'
                    temp_string1=temp_string+str(hidden1_list[i])
                    temp_string2=temp_string+str(hidden2_list[i])
                    tsne_string1+=temp_string1+'\n'
                    tsne_string2+=temp_string2+'\n'
                ...
                sub_loss2 += cross_loss * len(outputs)
                al_loss_total+=al_loss* len(outputs)
                un_loss_total+=un_loss* len(outputs)
                if global_step % self.opt.log_step == 0:
                    train_acc = n_correct / n_total
                    train_loss = loss_total / n_total
                    train_sub_loss1 = sub_loss1 / n_total
                    train_sub_loss2 = sub_loss2 / n_total
                    al_loss_avg=al_loss_total/n_total
                    un_loss_avg=un_loss_total/n_total
                    logger.info('loss: {:.4f}, supervised_loss: {:.4f}, cross_loss: {:.4f},acc: {:.4f}'.format(train_loss, train_sub_loss1, train_sub_loss2,train_acc))
                    al_un_string+='{:.4f}\t{:.4f}\t'.format(train_acc,train_loss)

            val_acc, val_f1, val_f1_0,val_f1_1, val_f1_2 = self._evaluate_acc_f1(val_data_loader)
            logger.info('> val_acc: {:.4f}, val_f1: {:.4f}, val_f1_0: {:.4f}, val_f1_1: {:.4f}, val_f1_2: {:.4f}'.format(val_acc, val_f1, val_f1_0, val_f1_1, val_f1_2))
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        with torch.no_grad():
            for i_batch, t_batch in enumerate(data_loader):
                ...
                polarity=t_batch['polarity'].detach().numpy().tolist()
                hidden1_list=hidden1.cpu().detach().numpy().tolist()
                hidden2_list=hidden2.cpu().detach().numpy().tolist()

                for i in range(len(polarity)):
                    temp_string=targets_dict[polarity[i]]+'\t'
                    temp_string1=temp_string+str(hidden1_list[i])
                    temp_string2=temp_string+str(hidden2_list[i])
                    tsne_string1+=temp_string1+'\n'
                    tsne_string2+=temp_string2+'\n'
        ...
        f1_0 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[0], average='macro')
        f1_1 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[1], average='macro')
        f1_2 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), labels=[2], average='macro')
        
       
        
        return acc, f1, f1_0, f1_1, f1_2
    ...
    def run(self):
        ...
        test_acc, test_f1, test_f1_0, test_f1_1, test_f1_2 = self._evaluate_acc_f1(test_data_loader)
        logger.info('>> test_acc: {:.4f}, test_f1: {:.4f}, test_f1_0: {:.4f}, test_f1_1: {:.4f}, test_f1_2: {:.4f}'.format(test_acc, test_f1, test_f1_0, test_f1_1, test_f1_2))
        ...
        f_out.write('test_acc: {:.4f}, test_f1: {:.4f}, , test_f1_0: {:.4f}, test_f1_1: {:.4f}, test_f1_2: {:.4f}'.format(test_acc, test_f1, test_f1_0, test_f1_1, test_f1_2)+'\n')
...
def main():
    ...
    parser.add_argument('--dataset', default='vast', type=str)
    ...
    parser.add_argument('--con_loss_weight_cross', default=1.5, type=float)
	...
	'dt_hc': {
        ...
        'cvs_aet': {
            'train': './datasets/o_cvs_aet',
            'test': './datasets/cvs_aet',
     },
        'ci_esrx': {
            'train': './datasets/o_ci_esrx',
            'test': './datasets/ci_esrx',
      },
        'antm_ci': {
            'train': './datasets/o_antm_ci',
            'test': './datasets/antm_ci',
      },
        'aet_hum': {
            'train': './datasets/o_aet_hum',
            'test': './datasets/aet_hum',
      },
        'vast': {
            'train': './zeroshot_vast/lda_vast_train.raw',
            'dev': './zeroshot_vast/vast_dev.raw',
            'test': './zeroshot_vast/vast_test.raw',
      },
      'few_vast': {
            'train': './zeroshot_vast/lda_vast_train.raw',
            'dev': './zeroshot_vast/few_vast_dev.raw',
            'test': './zeroshot_vast/few_vast_test.raw',
      },
      'all_vast': {
            'train': './zeroshot_vast/lda_vast_train.raw',
            'dev': './zeroshot_vast/all_vast_dev.raw',
            'test': './zeroshot_vast/all_vast_test.raw',
      },
   }
    input_colses = {
    ...
    opt.newname='lda_vast_'+str(opt.seed)
    ins = Instructor(opt)
    ins.run()
